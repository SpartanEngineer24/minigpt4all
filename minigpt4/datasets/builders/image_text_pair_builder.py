...
@registry.register_builder("cc_sbu_multi")
class CCSBUMultiBuilder(BaseDatasetBuilder):
    train_dataset_cls = CCSBUMulti

    DATASET_CONFIG_DICT = {
        "default": "/ibex/user/hansh/MiniGPT-4_multilingual/minigpt4/configs/datasets/cc_sbu/cc_sbu_multi.yaml",
    }

    def build_datasets(self):
        logging.info("Building CC_SBU multi-language dataset...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()
...
class CambrainBuilder(BaseDatasetBuilder):
    ...
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/cambrian/cambrian.yaml",
    }    
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building Cambrian dataset...")
        self.build_processors()
        build_info = self.config.build_info
...
@registry.register_builder("cambrian_text")
...
@registry.register_builder("llava_multi")
class LlavaMultiBuilder(BaseDatasetBuilder):
    train_dataset_cls = Llava_Multi
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/llava/llava_multi.yaml",
    }    
    def build_datasets(self):
        logging.info("Building Llava-Multi dataset...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()        # create datasets
        dataset_cls = self.train_dataset_cls
...
@registry.register_builder("llava_simple")
class LlavaSimpleBuilder(BaseDatasetBuilder):
    train_dataset_cls = Llava_Simple
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/llava/llava_simple.yaml",
    }    
    def build_datasets(self):
        logging.info("Building Llava-Simple dataset...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()        # create datasets
        dataset_cls = self.train_dataset_cls
...
@registry.register_builder("xstorycloze")
class XStoryClozeBuilder(BaseDatasetBuilder):
    train_dataset_cls = XStoryCloze
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/text_only/xstorycloze.yaml",
    }    
    def build_datasets(self):
        # at this point, all the annotations and image/videos should be all downloaded to the specified locations.
        logging.info("Building XStoryCloze dataset...")
        self.build_processors()
        build_info = self.config.build_info
        datasets = dict()        # create datasets
        dataset_cls = self.train_dataset_cls
...
class ArtelingoMultiBuilder(BaseDatasetBuilder):
    ...
    DATASET_CONFIG_DICT = {
        "default": "/ibex/user/hansh/Evaluation/artelingo/artelingo.yaml",
    }

    def build_datasets(self):
        logging.info("Building Artelingo Multi datasets...")
        self.build_processors()
        ...
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            ann_paths=[os.path.join(storage_path, 'multilingual_train_metadata_25.csv')],
